        ...
        @foreach ($agents as $key => $item)
            ...
            @isset($item['subteams'])
                @foreach ($item['subteams'] as $subteams)
                    @isset($subteams['sub'])
                        <tr class="subteam-color" data-id="{{ $subteams['sub']->code }}" data-parent="{{ $key + 1 }}"
                            data-level="2">
                            <td>
                        ...
                        @foreach ($subteams['users'] as $i => $subteam)
                            ...
                            <tr class="clickable" data-id="{{ $subteam->code }}" data-parent="{{ $key + 1 }}"
                                data-level="3">
                                <td>
                                    <div data-column="name"><i class="fas fa-minus fa-lg"></i></div>
                                </td>
                                <td>{{ $subteam->code }}</td>
                                    ...
                                    <ul class="m-auto p-0 list-unstyled">
                                        <li class="dropdown action-menu">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a href="{{ url('/users/') }}/{{ $subteam->id }}/edit"
